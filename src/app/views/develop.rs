...
use iced::widget::{button, checkbox, column, container, row, scrollable, slider, stack, text, Image, Container};
...
fn view_sidebar(editor: &RawEditor) -> iced::widget::Column<'_, Message> {
    ...
    sidebar.push(button(row![text(ui::icons::SAVE).font(ICON_FONT).size(14), text(" Export Image").size(14)].spacing(5).align_y(Alignment::Center)).style(ui::styles::AccentButton::style).on_press(Message::OpenExportModal).padding(12).width(Length::Fill)).spacing(10).padding(15)
}

fn prepare_preview(editor: &RawEditor) -> (Option<iced::widget::image::Handle>, Option<Element<'_, Message>>) {
    match &editor.editor_readiness {
        ...
        EditorReadiness::Ready(_) => {
            if let (Some(ctx), Some(resources)) = (&editor.gpu_context, &editor.image_resources) {
                let mut params_to_render = if editor.show_before { crate::core::types::EditParams::default() } else { editor.current_edit_params.clone() };
...
fn view_main_content<'a>(editor: &'a RawEditor, image_handle: Option<iced::widget::image::Handle>, overlay_content: Option<Element<'a, Message>>) -> Element<'a, Message> {
    ...
    let interactive_image = mouse_area(stacked_image)
        .on_scroll(|delta| { let zoom_delta = match delta { ScrollDelta::Lines { y, .. } => y * 0.1, ScrollDelta::Pixels { y, .. } => y * 0.01 }; Message::Zoom(zoom_delta, iced::Point::new(-1.0, -1.0)) })
